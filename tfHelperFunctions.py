...
def conv_layer(input_tensor, filter_height, filter_width, in_channels, out_channels, strides, padding, layer_name,
               act=tf.nn.relu):
    with tf.name_scope(layer_name):
        with tf.name_scope('window'):
            conv_filter = tf.Variable(tf.random_normal([filter_height, filter_width, in_channels, out_channels]),
                                      name=layer_name+'filter')
...
def max_pool_layer(input_tensor, ksize, strides, padding, layer_name):
    with tf.name_scope(layer_name):
        max_pool = tf.nn.max_pool(input_tensor, ksize=ksize, strides=strides, padding=padding,
                                  name=layer_name + 'max_pooling')
        return max_pool
